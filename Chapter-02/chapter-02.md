...
The first company to adopt systemd was Red Hat.  Debian was the last holdout and they had a sprited debate, which led to a number of resignations and a split within the community over the issue.  Some Debian developers left and went on to form a distro called [Devuan](http://www.devuan.org "devuan")--which is focused on removing all systemd and udev dependencies from a Debian based Linux distro.  
